...
class NBDTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p) {
  ...
  val pr_array     = widthMap(w => Cat(Fill(nPhysicalEntries, prot_r(w))   , normal_entries(w).map(_.pr).asUInt) & ~(ptw_ae_array(w) | final_ae_array(w)) & nacc_access_array(w) & naccRootReadArray(w) & naccTagReadArray(w))
  val pw_array     = widthMap(w => Cat(Fill(nPhysicalEntries, prot_w(w))   , normal_entries(w).map(_.pw).asUInt) & ~(ptw_ae_array(w) | final_ae_array(w)) & nacc_access_array(w) & naccRootWriteArray(w) & naccTagWriteArray(w))
  val px_array     = widthMap(w => Cat(Fill(nPhysicalEntries, prot_x(w))   , normal_entries(w).map(_.px).asUInt) & ~(ptw_ae_array(w) | final_ae_array(w)) & nacc_access_array(w) & naccTagExecuteArray(w))
